public class Main {
    ...
    public static void main(String[] args) {
        ...
        indvotes = isDuplicate(indvotes);

        votenum(indvotes, 0);

        for (int i = 0; i < candidates.length; i++){

            votecounter(indvotes, candidates, i);
    ...
    public static int[][] parse(String data) {
        ...
        String[][] allvotesstringarr = new String[5][250];

        String[] splitvotes = data.split(";");

        for (int i = 0; i < splitvotes.length; i++){

            allvotesstringarr[0][i] = splitvotes[i].substring(0, 6);
            allvotesstringarr[1][i] = splitvotes[i].substring(7, 8);
            allvotesstringarr[2][i] = splitvotes[i].substring(9, 10);
            allvotesstringarr[3][i] = splitvotes[i].substring(11, 12);
            allvotesstringarr[4][i] = splitvotes[i].substring(13);

        }

        int[][] allvotesintarr = new int[5][250];

        for (int j = 0; j < allvotesintarr.length; j++){

            for (int k = 0; k < allvotesintarr[j].length; k++){

                allvotesintarr[j][k] = Integer.parseInt(allvotesstringarr[j][k]);

            }

        }

        return allvotesintarr;

    }


    public static int[][] isDuplicate(int[][]indvotesintarr) {

        for (int i = 0; i < indvotesintarr[0].length; i++){

            for (int j = 0; j < indvotesintarr[0].length; j++){

                if (indvotesintarr[0][i] == indvotesintarr[0][j] && i != j){

                    for (int k = 1; k < indvotesintarr.length; k++){

                        indvotesintarr[k][i]= 0;
        ...
        return indvotesintarr;

    }

    public static void votenum(int [][] indvotes, int i) {

        int votenum = 0;

        for (int j = 0; j < indvotes[i].length; j++){
        ...
        System.out.println("There were a total of " + Integer.toString(250 - votenum)+ " votes.\n");

    }



    public static void votecounter(int[][] indvotes, String[][] candidates, int a) {

        int[] votecount= {0, 0, 0, 0, 0};

        for (int i = 0; i < votecount.length; i++){

            for (int j = 0; j < indvotes[a].length; j++){

                if (indvotes[a + 1][j]== i) votecount[i]++;

            }

        }

        System.out.println("For the office of " + candidates[a][0] + ":");

        for (int k = 1; k < candidates[a].length; k++){

            System.out.println(candidates[a][k] + " received " + Integer.toString(votecount[k]) + " votes.");
        ...
        for (int l = 0; l < votecount.length; l++){
        ...
        System.out.println(candidates[a][winner] + " won!\n");
